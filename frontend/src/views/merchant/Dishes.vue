<template>
  <div v-loading="loading">
    ...
    <el-card>
      ...
      <div class="table-scroll">
        <el-table :data="dishes">
          ...
          <el-table-column label="整份价(元)" width="110">
            <template #default="{ row }">{{ formatMoney(row.price) }}</template>
          </el-table-column>
          <el-table-column label="半份" width="140">
            <template #default="{ row }">
              <span v-if="supportsHalf(row)">
                开启 · {{ formatMoney(row.halfPrice) }} 元
              </span>
    ...
    <el-dialog v-model="showStockDialog" title="修改库存" :width="isMobile ? '92vw' : '320px'">
...
const supportsHalf = (dish) => Number(dish?.allowHalfPortion || 0) === 1 && Number(dish?.halfPrice || 0) > 0
...
const loadData = async () => {
  loading.value = true
  try {
    const [categoryRes, dishRes] = await Promise.all([listCategories(), listDishes()])
